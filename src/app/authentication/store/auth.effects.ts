          ...
          return this.SigninService.login(data).pipe(
            map((data) => {
              ...
              return this.usuarioService.geDatatUsuario().subscribe((data) => {
                return this.storeAuth.dispatch(
                  authActions.LoginSuccess(data, true, true)
                );
              });
            }),
            catchError((error) => {
      ...
      this.action$.pipe(
        ...
        exhaustMap((): any => {
          ...
          if (!token) {
            this.router.navigate(['/login']);
            return of(
              authActions.AutoLoginError({
                error: 'No existe ningún usuario autenticado',
              })
            );
          }
          ...
          if (!tokenExpired) {
            return this.usuarioService.geDatatUsuario().pipe(
              map((user) => {
                return this.storeAuth.dispatch(
                  authActions.LoginSuccess(user, true)
                );
              }),
              catchError((error) => {
                this.router.navigate(['/login']);
            ...
            return of(
              authActions.AutoLoginError({
                error: 'La sesión ha expirado, por favor inicie sesión',
              })
            );
          }
        })
